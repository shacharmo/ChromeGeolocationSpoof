...
function returnResult(event, result) {
  window.postMessage({
    type: event.data.callback || (event.data.type + 'Callback'),
    id: event.data.id,
    result
  }, '*');
}

function returnError(event, error) {
  window.postMessage({
    type: event.data.callback || (event.data.type + 'Callback'),
    id: event.data.id,
    error
...
function getRoute(request): Promise<string> {
  ...
  const handler = new Promise((resolve, reject) => {
    ...
    switch (request.type) {
      ...
      case 'setLocation':
        ...
        return resolve();
      case 'getRoute':
        getRoute(request).then(result => resolve(result)).catch(err => reject(err));
        break;
      case 'playRoute':
      case 'pauseRoute':
      case 'resetRoute':
...
window.addEventListener('message', function (event) {
  ...
  switch (event.data.type) {
    case "getRoute":
      getRoute(event.data)
        .then(result => returnResult(event, result))
        .catch(err => returnError(event, err));
      break;
  }
}, false);